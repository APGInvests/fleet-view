...
**Scan-hours (shipped 2026-09-07, `scan-hours`) — opt-in meter capture in the batch unload loop.** Bourbon load-in proved the loop (25 units / 11 min / zero hours): fast BECAUSE it asks nothing. Andy wants hours at arrival when the tech has time — dispatch-by-runway from the office, no more units arriving already over service. Design: a **per-session mode, never a field in everyone's path** — `scanAskHours` toggle on the scan sheet (default OFF, resets every boot). On: scan → destination → `scanHoursSheet` for THAT machine (serial in the title, one meter field; twins get A/B) → Save or Skip → scanner. Contracts (`fv_inv_addflow.js` scan-unload group, suite 1051): the **move is recorded BEFORE the hours ask** (hours never gate an arrival); field never prefilled (observation rule); blank/Skip writes nothing; a value writes a **'Meter reading' check** (same convention as the edit-form twin fields — observation with a name on it, never a bare column write) + bumps `currentHours`/`updatedAt` on non-twins; twin package hours never merged onto the unit. Harness note: new mutable top-level bindings must be registered in `LIVE_BINDINGS` (fv_harness.js) or `app.live.X` reads undefined. The same toggle at teardown scan-into-shop is what makes the yard pile sortable by runway — worth telling the crew.

**Fuel in inches (shipped 2026-09-11, `fuel-inches`) — the dip-stick toggle.** Bourbon techs dip XQ-500-class tanks with a paint stick marked in EIGHTHS of an inch (11″ = full) while others read the gauge in % — both must land in one log. **Storage is and stays `fuel_level_pct`; inches exist only at the display/input layer** (zero schema, zero row edits, pure client constants, works offline). `TANK_SPECS` (keyed like `SPEC_RANGES`, normalized make+model) carries `{gallons:750, fullInches:11}` — **750 is the fleet fill record; the plate's 780 is UL-142 shell capacity**; capacity never enters the conversion (both units are fractions of full), it's informational only. Targeting is model-string **OR the `TANK_SERIALS` exact-serial allowlist** — 12 blank-model units the fleet capacity record puts on the same tank (T4A00750–753/756–759, GG500104/109/110/111). **Exact serials, never a prefix** (T4A00754-55 is a 1250 TwinPak sharing T4A007), never kw/kVA (32 HiPower 625s share kw=625 on 600-gal tanks). B3G00213 excluded until its capacity is verified; adding a tank family = one `TANK_SPECS` entry, same doctrine as the range flags. The toggle is **per show** (`fleetview_fuelunit_v1` localStorage map keyed by the unit's stamped show; only `'in'` is stored, % deletes the key so default-% holds by construction; off-show units always read %) and governs **input and display**: the check-form fuel field becomes `Fuel (in)` with `step=0.125 max=11` (mid-form toggle converts a typed value in place — never rebuilds the sheet), `saveVitals` converts inches→pct at save, and every render surface (card fuel bar label, low-fuel alert reason, latest-check card, history lines) shows stick fractions (`1⅜″`, `5½″` — `fmtIn`). **Bar geometry, colors, the <20% low-fuel rule, and the lowfuel filter all stay pct** — labels convert, logic never does. Eighths round-trip losslessly through integer pct (marks sit 100/88≈1.14% apart; proven over all 89 values). 74 invariants in `fv_inv_fuelunit.js` (suite 1125); the two `fv_inv_checkform.js` DEF-grid anchors were updated for the label's new `id="v_fuelLb"` (markup anchor only — the grid2/grid3 contract is untouched).

**One-field job label (built 2026-09-21, `job-label-one-field`) — Name+Placement collapsed, size-locked titles.** Field feedback: three per-job fields confused techs; a typed Name replaced the kVA in the card title, and placement rendered twice (title + pin line). **Data verdict** (live, all shows, 189 job-meta entries): Name-only **0** — never once used alone; Placement-only **167**; both **4** (2.3% of 171 labeled — Lolla "Main Medical 480v"/"Grove", two Portola twins whose name contained the placement, one orphan key on a deleted show); neither 18 (note-only/empty, 13 Bourbon). So: **one field, stored in `area`**, edited as **"What it's powering"** on the Placement tab (`jm_area` id kept; `jm_name` input gone; "Save job label" + "Stays on this job only" wording kept). **`name` is legacy, read-only, never written again**: it displays forever through the merge rule — *containing string wins (case-insensitive), else `name — area`* — and drops off an entry on its next save (the prefill shows the merged text, so nothing typed is ever lost; zero data migration, zero row writes). **Size-locked title doctrine:** every card title is `sizeLabel(u)` first — `150 kVA · Main Stage` — in ALL variants: healthy, down (issue moves to its own `⚠` line with no label text in it), and transit. Nothing a tech types can replace the size. The label appears exactly ONCE per card: labeled units drop the pin line entirely; unlabeled units keep the location line as before. New helpers `sizeLabel`/`jobLabel` (index ~:936); **`assetLabel`/`placeOf`/`jm` are UNTOUCHED** — sacred fv_smoke pins them and passes verbatim; `assetLabel` now has zero production callers (legacy-pinned; retiring it means amending fv_smoke — Andy's call, some day). **Every `name`-key reader routed** (Andy's audit): `byKva` same-size tiebreak → jobLabel (placement-labeled units, 98% of real labels, now tiebreak too); jobSearch → jobLabel (legacy names stay findable); map popup → jobLabel (was `area||name` — third inconsistent ordering, gone); `fv_sitemap.js` placementText → same merge rule duplicated with pointer comment; `fv_archive.js` roster.csv keeps raw `job_name`/`placement`/`job_note` (archival fidelity) + new **`job_label`** merged column, dictionary updated; **Report (`buildReportText`) reads NO per-job labels and never did** (`#serial [make model]` + status) — unchanged, adding labels there is a separate decision. Known wart, deliberate: all-empty save still stores `{area:'',note:''}` (an empty key counts as archive roster evidence — changing that is an archive-semantics call, not this build). Invariants incl. the four real production pairs as fixtures in `fv_inv_joblabel.js` (suite 1306).

**Add-sheet steps (built 2026-09-21, `add-sheet-steps`) — field feedback on the serial-first sheet.** Four changes, one build. **(1) Three-step intro**, sun-readable by Andy's spec (full 16px ink, never muted — only helper lines go muted): `1 · Type the serial / 2 · Already in the fleet? Tap it — it moves onto <job>. No duplicates. / 3 · Not found? Add it as a new asset.` **(2) "+ Add new asset" earns its loudness** (`#addNewBtn` + `addNewVis()`): hidden until a search exists — a loud add button before searching is how duplicates happen — ghost while match cards are on screen (the cards are the suggestion), **primary only on a miss**, exactly when the locked results copy says "Tap + Add new asset". No test pinned the old always-primary button; the searchFleet copy contracts all stand. **(3) Hours toggle is a question**: `⏱ Log hours too?` / `⏱ Logging hours after each unit · ON`, with the non-negotiable stated in-UI beneath it: *"Optional — the unit is received either way. Skip anytime."* Same id, same in-place patch. **(4) Per-JOB memory** (`fleetview_scanhours_v1`, fuelUnitMap self-pruning pattern — only ON stored, OFF deletes the key): `openScan` hydrates the global from the map for the open job, `scanAskHoursT` persists in job mode; **a job never set defaults OFF by construction**; fleet-mode stays session-only. This supersedes the scan-hours entry's "resets every boot" (comment in `fv_inv_addflow.js` updated; its assertions untouched — the global still boots false). Receipt-before-hours and Skip-writes-nothing re-pinned with persistence ON. Invariants in `fv_inv_addsheet.js` (suite 1265).

**Add-sheet trim (built 2026-09-21, `add-sheet-trim`) — hours card, barcode off the sheet, asset-photo nudge.** Data-first pass over live photos + scan usage. **(1) Hours question is ONE card with a real switch:** `#hrsCard` holds the `⏱ Log hours too?` label (`#scanHrsLb`, flips to `⏱ Logging hours after each unit` when ON), a `role="switch"` toggle (`#scanHrsTgl`, new `.sw` CSS control — first real switch in the app; reuse it for future on/offs), and the *"Optional — the unit is received either way. Skip anytime."* line inside the same card. Per-job memory, default OFF, and in-place patching (never rebuild the sheet) all stand from `add-sheet-steps`. **(2) Barcode entry point removed from the add sheet** — zero scans ever recorded in production (1 unit fleet-wide even has a `tag_id`); `#scanCamBtn`/`#camWrap`/`#reader` no longer render. The scanner CODE is kept dormant (`scanCamT`/`startScanner`/torch + the html5-qrcode CDN tag) for the parked QR-label plan — do not delete, do not re-add the button without that plan. **(3) Two photo kinds, two homes, confirmed by live data** (2026-09-21: 39 placement rows `kind:'photo'` incl. Sea Hear Now 9-19; 25 asset photos on 21 units incl. a 7-unit Portola batch 9-20 — techs already use both surfaces unprompted): placement photos stay on the job (Placement tab / movement rows); asset photos (size, chassis, forklift pockets, condition) live on `units.photos`, home = **Info tab**, which gains a muted empty-state nudge naming those shots when a unit has none — Info-tab-only, no card chips, no alerts, photos stay optional (§8 rule 6). **The add sheet never mentions photos** (pinned) — the receipt loop stays friction-free. §7's "photos empty by design" note is now historical: organic asset photos exist since 2026-08. Invariants amended/added in `fv_inv_addsheet.js`, `fv_inv_addclarity.js`, `fv_inv_photos.js` (suite 1319).

**Move-uuid-null (shipped 2026-09-21, `move-uuid-null`) — the U041866X stuck-sync fix.** Field incident, Meta Classic, Sep 21 12:41 PM: "↩ Back to fleet (unassign)" passes `''` as the destination id (`moveMenu` → `pickDest(unitId,'fleet','')`), so `doMove` wrote `locationId:''` on the unit and `toId:''` on its movement — Postgres refuses `''` for uuid (`invalid input syntax for type uuid: ""`), both rows correctly DEAD-quarantined, two "stuck" cards on the phone. Latent since the move menu existed; first tripped because unassign-to-fleet is rare. Fix is two-layered: **(1)** `doMove`/`doMoveScan` coerce the incoming id `||null` and all three movers (`doShip` too) stamp `fromId: u.locationId||null`, so local state is clean going forward; **(2)** `toRow` coerces `'' → null` on every uuid-mapped column via new per-table `uu` lists in MAPS — the choke point every write crosses. (2) is also the HEAL path: `retryDead` doesn't replay the stored payload, it un-parks and re-diffs from live state through `toRow`, so the two parked rows recover with one **Retry** tap after this build reaches the phone — no data re-entry. Coercion is uuid-only by explicit field list: text fields (`tag_id`, `notes`, `model`…) keep `''` untouched, so no serialization churn on clean rows (and cached SNAP fingerprints re-normalize through the same path on restore, per sync-hygiene). Invariants in `fv_inv_moveids.js` (suite 1229). Shipped together with `add-flow-clarity` (below).

**Add-flow clarity (built 2026-09-21, `add-flow-clarity`) — empty-state doctrine + serial-first add sheet + Day N fix.** Andy's field report: a new job showed a big inert plus with no guidance, the only instruction ("No units match. Tap + to add one") sat below the fold in filter language, and the add sheet was camera-first when no scannable labels exist on the machines. **Empty-state doctrine, now pinned:** an empty list says WHY — zero-added, search miss, and filter miss are three states with three copies, and every zero state names its next action on a real button (the Jobs-list pattern, now on job detail and the Fleet registry). Zero-unit job: search bar + filter chips HIDDEN (nothing to filter), `⚡ No generators on this job yet` + `+ Add the first generator` directly under the action row — "generator" deliberate in the first-run teaching line (crew word); working chrome keeps "unit". Filter miss names its filter (`FILTER_EMPTY` map) + `Show all`; search miss interpolates the escaped query. The oversized inert `IC.plus`/`.em` glyph is gone. **Add sheet is serial-first:** typing is the proven path (Bourbon 25/11min loop was typed; every dup incident a typo) — intro leads "Type the serial", field on top (autofocused, keyboard-hardened attrs), **camera collapsed behind `📷 Scan a barcode instead`** (`scanCamOpen`, session-only, boot-reset like `scanAskHours`): no camera activation or permission prompt for typers, one extra tap per session for scanners, then sticky through the batch-loop re-arms. Do not "improve" the camera back to always-on — the collapse IS the feature. Camera-failure copy now points at controls that exist ("Type the serial above" — the old copy referenced a nonexistent "Add manually" button). **Day N off-by-one fixed:** `dayOfShow` parsed date-only `start_date` as UTC midnight = previous local day west of UTC, so a job starting today read "Day 2" in Pacific and tomorrow's job read "Day 1"; now parses local noon (`+'T12:00:00'`, the house convention at the archive day-grouping and calendar init). The regression test runs the suite under `TZ=America/Los_Angeles` — the bug only reproduces west of UTC; keep the TZ prefix on suite runs. Harness: `scanCamOpen`/`fleetFilter`/`fleetSearch`/`fleetMake` registered in LIVE_BINDINGS. Invariants in `fv_inv_addclarity.js` (suite 1208).

**Twin sizing in kW (built 2026-09-22, `twin-size-kw`) — big iron speaks per-engine kW.** The NES and the crew both say "500 kW" and "Twin 400"; the app said "625 kVA" and "1000 kVA" — a translation the tech did in his head at every card. Now **big iron renders per-engine kW everywhere; small iron stays kVA; every size label carries its unit**. Conversion is **render-only ×0.8** (`kvaKw`): storage stays kVA in `units.kw` and `engines[e].kvaEach`, zero schema, zero row edits — a PF-convention change re-renders the whole fleet, edits nothing. **`sizeText`/`sizeLabel` (index ~:956) are the choke point** — card titles, fleet/near-match/add-sheet cards, search haystacks and the Info Rating row all read through them; `twinPerKva` resolves a twin's per-engine kVA (matched nameplates win; else `kw/2` for binding-pass stragglers — a blended line, see the symmetric-twin note below). **Sort is NES order** (`byKva`): per-engine size ascending, **single before twin at equal size** — the sequence a PM reads off the schedule. **Search:** all three haystacks (job, fleet, add-sheet) carry `sizeText` PLUS the legacy `kw + ' kva'` token — `400`, `twin`, `400 kw` and a month of `1000` muscle memory all land; **fleet search gained size tokens it never had.** Fleet/add-sheet deliberately omit `jobLabel` — those are registry surfaces, placement search stays a job-detail feature (a separate decision, §7 note stands). **Edit form: inputs stay kVA** (techs type what the nameplate says), with a live muted **kW echo readback** (`kwEcho` on big singles' `#f_kwEcho`; `kvaEcho` under the twin nameplate pair's `#f_kvaEcho` — `= Twin 400 kW` when matched, `= 400 kW + 300 kW` when not, never a fake Twin label). Echo is text only, never a control, never prefills, hidden when blank/garbage/zero; **the twin sheet deliberately has NO combined-kW echo** — "= 800 kW" is a number no surface may show. **Info tab gains a RATED SPECS block** (`specBlock`/`specRow`/`ratedA`): kW · kVA · A@208V · A@480V, derived at render from the stored nameplate kVA (never stored, never an input — a formula fix re-derives everything), `toLocaleString('en-US')`-pinned, big iron only; twins show **Per engine + "Combined (when paralleled)"**. `specBlock` assumes symmetric twins — an asymmetric `kvaEach` pair renders a blended per-engine line; revisit only if the binding pass ever records one. Invariants in `tools/fv_inv_twinsize.js` (91 assertions; `fv_inv_fleetcard.js` pins updated; suite 1416).

**Born-twin doctrine (Andy, 2026-09-22).** A twin pack is **BORN twin**: one container, one control panel, A and B engines, never split — Technogen TGDs, CAT twin packs. **XQ500/X5M units are ALWAYS singles**, even when married in pairs on a 40′ chassis: chassis pairings are temporary, can be split and re-paired with any other unit, and are **NEVER twin status**. Twin status = `engines` set, only ever for born twin packs. This **retires the three X5M entries** (`X5M00306`, `X5M0038`, `X5M00446`) from the §10 1b binding-pass expectation — they were chassis pairings, not conversion candidates. **Parked (Andy, 2026-09-22):** "highest leg % of rated amps" as a possible future secondary warning — NOT built; the load % method is unchanged. **Deliberate:** `tools/fv_sitemap.js` + `tools/fv_archive.js` stay kVA — client-sheet and archival conventions are separate decisions. **Intentionally excluded NES twins (Andy, 2026-09-22 — broken or rarely used; do not import):** 1624072-845823 (175 kW Twin, 208V only) · 0626113-845828 (175 kW Twin, 208V only, 30′) · 0691927-854743 (175 kW Twin, 208V only) · 0629257/58 (175 kW Twin, 208V only) · MS415044A/B (175 kW Twin, Prod Power, 208V only) · A06503/A06672 (180 kW Twin, 208V only, not for broadcast) · 9NR04789-95 (225 kW Twin, pintle hitch) · 9NR04792-93 (225 kW Twin, pintle hitch) · 0677177/78 (300 kW Twin, 208V only) · 0675783/84 (300 kW Twin, 208V only) · 0639342/0515754 (300 kW Twin, 208V only). Two NES twins ARE being added separately — 9NR05394-97 and 9NR05395-96 (200 kW Twins) — so this list plus those two reads as the complete NES twin population. **Ran 2026-09-22 (same day, Andy's go):** all four data fixes (TGD62501/04/07 kw 1257→1250, C5E02263-64 config→TwinPak, CATPMC15JXM00492 kw→null, three X5M configs→null) and both 9NR inserts verified in one read-back. **Parked: voltage-gated spec block.** The RATED SPECS block is voltage-agnostic — it shows A@480V on 208V-only machines (both 9NRs and the already-imported 396970/396971, `[NES] DC; 208 V only`). Mitigation today is the `[NES] 208V ONLY` note rendering on the same Info tab directly below the block; gating the 480V row needs a voltage-capability property (new scope, not this build).

**Hours-to-service runway (built 2026-09-22, `svc-runway`) — the PM placement question on the job card.** After a truck is unloaded, the card answers "how many hours does this unit have left before service" so high-runtime areas land on units with the most runway. A `🔧` chip leads the card's metaline — `🔧 210 h to service · 3d ago` — rendered ONLY when provable: a service-due target exists AND a non-voided hours reading (`reports.engine_hours` + `ts`) is timestamped AFTER the unit's latest real **arrival movement** onto this show. **Hours come from timestamped check records for singles AND twins — `units.currentHours` is never read here** (no timestamp of its own; `updated_at` moves on any edit). **Arrival = latest movement with `toType 'show'`, `toId` = the unit's show, `kind` null, from ≠ to** — pin-sets/placement captures (from = to) and `kind:'photo'` rows are never arrivals; the new-asset birth row (`fromType null`) is, so born-on-job units are covered with no special case. No target, no reading, or a pre-arrival reading = nothing — never invent an interval, never carry old hours (Bourbon reads blank until the crew trues up: correct, not a defect). **Age doctrine (Andy, 2026-09-22):** crews don't check units back out and jobs sit open for months, so the reading's AGE is the judgment surface — freshness-since-arrival is just the floor. Age always renders next to the hours; past **14 days** the wording drops the live claim for a dated fact (`210 h as of 26d ago`) but the number is **never hidden** — hours-to-service only shrinks between readings, so an old reading overstates runway and an old OVERDUE is certain, which is also why `OVER N h` stays red (`chip bad`) at any age. **Twins: lower-of-two, engine named** (`🔧 Gen B 120 h to service · 3d ago`), lanes via `engReports` (A inherits untagged, B never does), flat `serviceDueHours` on a twin ignored; per-engine detail stays on the Service tab. Day-one fleet (live, 2026-09-22): 99 of 198 units on non-archived shows show a chip — 12 twins lack per-engine due targets, 32 singles lack `service_due_hours`, 16 units' latest reading predates arrival, 1 unit (D1970162) has no arrival row; all render nothing, honestly. **Nothing here blocks receiving or makes hours required** (receipt-before-hours invariants stand). Surfaces: job detail + Incoming + Alerts cards (all `unitCard`); the Fleet registry card deliberately unchanged; transit units get no chip by construction (`unitShowId` requires `locationType 'show'`). **Killed, parked: sort/filter by hours left** — the NES size sort is settled and the chip's data only exists where the scan-hours toggle was used; re-open only if a PM asks for ranking on a >30-unit show with fresh hours. **Deliberate scope: the Alerts service section and the `service` filter chip stay `currentHours`-based** — switching them to the freshness rule would REMOVE units from alerts when data is missing, inverting bias-to-flagging; singles' `currentHours` auto-tracks every check so they stay consistent. Also in this build: **`openServicePlanner` deleted** (dead code, zero callers, the §6 service-planner cut — an invariant now keeps the identifier out of the source). **⚠ Load-bearing (Andy, 2026-09-22): the arrival movement — unit onto job, the receipt — is now infrastructure for this feature.** The return/checkout direction is separately unused and could be trimmed someday, but the arrival record must stay. Invariants in `tools/fv_inv_svcrunway.js` (44 assertions; suite 1460). Plan: `docs/plans/2026-09-22-svc-runway.md`.

**Fuel echo (shipped 2026-09-11, `fuel-echo`).** The inches keypad wants `4.125` when the stick says 4⅛ — the tech converts in his head with no confirmation he got it right, and off-mark values snap silently at save. Now a live line under the fuel field (`#v_fuelEcho`, the `v_loadCalc` inline-helper pattern, driven by `fuelEcho()` on input and by the mid-form toggle) reads **`= 4⅛″ · 38%`** — the stick mark the decimal means, the snap made visible (`1.3` → `= 1⅜″ · 12%`), and the pct that will actually be stored. Text only, never a control, hidden (`display:none`) when empty or in % mode — zero height for % techs, and it never suggests a value: the observation rule guards the input; this is readback of what was typed. Echo group in `fv_inv_fuelunit.js` (suite 1139). The tap-target follow-up shipped same day (`fuel-marks`, below).

**Fuel stick-mark tap targets (shipped 2026-09-11, `fuel-marks`) — Andy's pick: 4×2 grid.** Problem (Andy's framing, distinct from the conversion problem the echo killed): **one-handed gloved entry in the sun while holding a dip stick** — typing `4.125` on a keypad is the failure mode. The stick is painted at TANK-eighths, 1⅜″ apart, so there are exactly eight marks: 1⅜ · 2¾ · 4⅛ · 5½ · 6⅞ · 8¼ · 9⅝ · 11 — **eight targets, never collapsed to quarters** (that would discard half the stick's resolution). Layout per the measured design pass: **4×2 grid of gauge-chip-style buttons at `min-height:44px`** (`#v_fuelMarks`, ~104px added in inches mode) — chosen over 8-across segwrap (only 36–43px per target on ≤375px phones, and shared edges turn a gloved miss into a silent ±⅛-tank neighbor-mark log) and over a scrollable row (upper half of the tank behind a swipe, in exactly the one-handed scenario). A tap fills `#v_fuel` (`fuelMarkT` — the tech's own act, not a prefill; the keyboard never opens on-mark) and fires the echo; **highlight follows the field, never the reverse** (`fuelMarksSync`: an off-mark typed value lights nothing). Typing stays for between-mark and below-1⅜ readings. Inches mode only: `display:none` in % mode on spec units (flipped live by the mode toggle, no sheet rebuild), zero markup on non-spec/off-show units. Storage path untouched — tap 5½ → field `5.5` → stored `50`. Marks group in `fv_inv_fuelunit.js` (suite 1162). Crew verdict expected from Head Trip.

**Sync hygiene (shipped 2026-09-07, `sync-hygiene`) — the mass false-"overwritten" fix.** Incident: on 2026-08-13 4:29 PM a browser holding a pre-`has_def` durable cache opened the post-`has_def` build and parked **274 untouched units** in SYNC_LOST at one page-load. Mechanism: growing MAPS changes `toRow`'s serialization (`undefined`→`null`, key always emitted), so every cached SNAP fingerprint mismatched → all rows read dirty → LWW tie (`sv.updatedAt >= o.updatedAt` — equality parks) → whole fleet "overwritten by someone else" with nobody involved. Structural, would recur on every future column. Fix: `hydrateFromCache` now **normalizes cached SNAP strings through the current schema** (`toRow(fromRow(JSON.parse(s)))`) — key growth/removal/order washes out, real VALUE edits still differ, so dirt is never laundered; malformed strings are left as-is (conservative: reads dirty, never clean). Plus: **Dismiss all (N)** button on the overwritten list (274 taps is not a workflow), and `dismissLost`/`dismissAllLost` both `persistCache()` so dismissals survive a reload. Invariants: `fv_inv_offline.js` "cache restore: schema growth never manufactures conflicts" + "dismiss-all" groups (suite 1022) — including the contracts that a locally-newer real edit still wins the merge and a server-newer real conflict still parks exactly that row.
...
### Job detail
Header buttons: Map · Log · Report. Search + filter chips (All / Big iron / Small iron / Down / Low fuel / Service). Each card's metaline leads with the **hours-to-service runway chip** when provable (`🔧 210 h to service · 3d ago` — see the `svc-runway` entry in §3). Unit cards sorted by **kVA ascending, with hard-down units forced to the bottom**, tiebreaking on the **job label when set, else serial** (same-kVA units are indistinguishable by serial alone) (crews doing rounds need runnable units on top; a known-broken unit is noise). Card swipe left = **Off job**; swipe right = its map pin.
...
**paneInfo's Weight row still uses locale-unpinned `toLocaleString()`** (pre-existing; the new `specRow` is pinned `en-US`) — candidate for the same pin later.
