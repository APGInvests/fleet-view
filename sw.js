...
self.addEventListener('fetch', (e) => {
  ...
  /* TILE SERVERS NEVER ENTER THE SERVICE WORKER CACHE (HANDOFF §8 rule 13).
     ...
     Applies to USGS imagery, the OSM street tiles, AND the NAIP Plus dynamic
     renders — those are the worst candidates of all: exportImage responses are
     large and uncacheable-by-design (every pan is a fresh bbox, so cached
     entries almost never re-match and only bloat the cache toward eviction).
     Do not add tile hosts to CDN_HOSTS — guarded by tools/fv_inv_sw.js. */
  if (url.hostname === 'basemap.nationalmap.gov' || url.hostname === 'imagery.nationalmap.gov' || url.hostname.endsWith('tile.openstreetmap.org')) return;
